...
import me.hoonti06.deliverytracker.step1.exception.DocumentParseException;
...
public class LotteGlogisParsedDocument {
  ...
  private DeliveryLastStatus parse(final Document doc) {
    ...
    if (CollectionUtils.isEmpty(statusTableRows)) {
      throw new DocumentParseException("statusTableTd IS EMPTY");
    }
    ...
    if (cells.size() < 4) {
      throw new DocumentParseException("미등록운송장");
    }
  ...
  private DeliveryLastStatus parseLastStatus(final Element lastRow) {
    ...
    if (lastRowCells.size() < 4) {
      throw new DocumentParseException("cell count is less than 4");
    }

    final Element statusCell = lastRowCells.get(0);
    if (!statusCell.hasText()) {
      throw new DocumentParseException("status cell has no text");
    }
    ...
    try {
      ...
      throw new DocumentParseException(e.getMessage(), e);
    }
  }
}
